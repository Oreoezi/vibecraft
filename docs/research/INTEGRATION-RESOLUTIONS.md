...
| Child-process singleplayer versus in-process hosting | One host-agnostic `ServerCore`; supervised child-loopback is the selected Windows/Linux desktop default. The embedded adapter is retained only as a conformance/fallback path and must produce equivalent protocol/authority traces. | Child lifecycle/package gate; embedded conformance fixture |
...
| ASSET-03 versus ASSET-04 GLB/animation contracts | Neither brief selects GLB/glTF as a public pack contract. V1 uses VibeCraft `RigProfile` compatibility and fixed built-in clips; the native voxel model/rig source format is a bounded spike. GLB/glTF remains historical/offline-import research only. | Model/rig format spike |
| Resource-pack dependency graph versus overlays | A selected profile is an explicit low-to-high list of exact resource-pack artifacts. For each `AssetKey`, the last selected pack defining the whole asset wins. Resource packs have no `requires`, optional edges, dependency DAG, or deep merge. | Pack-stack validation fixture |
...
- Native model/rig source format after the bounded spike.
