...
| **Post-v1 coordination** | Future work must respect a first-playable seam if it is eventually scheduled. | No G0–G5 edge and no permission to implement a future subsystem now. |
...
The hard edges are exactly `G0→G1`, `G1→G2`, `G1→G3`, `G1→G4B`, `G3→G4A`, and `{G2,G3,G4A,G4B}→G5`. G2, G3, and G4B may proceed in parallel after G1. G4A uses synthetic section payloads and therefore does not wait for G2 or G4B. G4B consumes G1 snapshots and does not wait for transport or persistence. Their real interactions are validated only at G5.
...
| **G1 — core data** | G0 | Canonical signed 3D `BlockCoord`, `SectionCoord`, and dimension-bearing `SectionKey`; floor division/local-index rules; selected section side and indexing order; stable namespaced content identity separated from world-local/runtime IDs; missing content distinct from air; checked revision domains; deterministic logical serialization and record-key encoding; one 20 Hz `WorldTick` owner and phase vocabulary. | **E1:** measure 16³ vs 32³ and adaptive vs simpler storage. Record one section representation disposition before user worlds exist. | Persistent user worlds; Godot types in world identity; general generator epochs; structures; plugin data. |
...
| **G3 — authority and movement** | G1 | Server authority matrix; shared voxel-controller and collision rules version; authoritative `WorldTick` distinct from wrapping client input sequence; bounded redundant input and missing-input rule; one causal action timeline; `ActionId`, expected revision, idempotent current-time block result; owner acknowledgement/reconciliation and remote interpolation semantics; bounded history and queues. | **E3:** blind-test 20 Hz prediction under G0 impairment. Only a recorded failure unlocks one exactly nested 40 Hz controller experiment. Use in-memory transport plus deterministic impairment. | GNS or another native transport; Protobuf field-number freeze; combat rewind; support lease; speculative collision; whole-world tick profiles. |
| **G4A — transport, trust, interest** | G3 | Transport-independent application envelope; realtime/control/bulk semantics; explicit ownership and backpressure results; message/count/byte/work bounds; handshake and negotiated protocol major/capabilities used by the slice; authenticated channel/server identity appropriate to G0 exposure; section baseline/epoch/revision and interest semantics that never rely on cross-lane arrival order. | **E4:** package and fault-test pinned GNS first; `greenlight`, `revise`, or `reject`, then one focused alternative if needed. Measure actual compressed payloads, queue age, settling, churn, malformed admission, and amplification. | Custom UDP fallback; public-anonymous promises; relays/accounts/discovery; hash-as-attestation; fixed transport lane numbers as gameplay semantics. |
| **G4B — Godot client, renderer, base pack** | G1 | Godot-free core/client boundary; immutable section-render snapshot; mesh product key + revision + lifetime/epoch; bounded build/upload/disposal ownership; render-local checked conversion and floating origin; minimal block-render template/material ID; fixed small terrain surface classes; resource-only `.vcpak` path, identity, and bounded VFS contract exercised by one first-party pack. | **E6:** hidden-face baseline vs measured greedy optimization; node/`ArrayMesh` vs low-level renderer only if needed. **E7:** validate one minimal base pack, canonical paths, logical digest, artifact digest/length, and reproducible resolution. | Far LoD; light pages; volumetrics/GI/OIT/refraction; GLB rigs; animation graphs; general pack dependency ecosystem; procedural runtime assets. |
| **G5 — integrated first playable** | G2 + G3 + G4A + G4B | Packaged lifecycle contract for connect/start, enter, move, stream, edit, save, stop, restart, and reconnect; equivalent local/remote authority paths; selected desktop host adapter; explicit save/quit status; cross-branch diagnostics and bounded-resource stop rules. No new world, wire, or pack primitive should be invented at integration without sending the affected branch back to its owning gate. | **E5:** compare child-loopback and embedded host adapters using startup, memory, crash containment, packaging/signing, pause, save, orphan cleanup, and protocol-trace equivalence. Run the declared acceptance and soak/fault matrix. | Survival milestone features; public mods; redstone; advanced rendering; additional dimensions; claims based on an individual branch demo. |
...
| --- | --- | --- | --- |
| G0 | G1–G5 | Acceptance fixture and exclusions | Every numeric result names this fixture. |
...
| `NET-04 ↔ NET-06` and `NET-04/05/06` | G3 owns 20 Hz world time and current-time block action causality. G4A interest management consumes revisions and traffic classes. Combat rewind/support grace remain off. |
| `RENDER-03 ↔ RENDER-07` | Remove the pair from G0–G5. Ordinary fog closes the measured full-detail radius; far LoD coordination is post-v1. |
...
| G5 | [`ARCH-04`](../decisions/ARCH-04-singleplayer-server-lifecycle.md) plus all branch references above | Host-neutral lifecycle and measured child/embedded selection; not a preselected topology. |
...
| General pack dependencies/overrides, Minecraft conversion, GLB rigs, animation graph, hot reload, procedural authoring | After first-party content demonstrates a requirement | Resource-only `.vcpak`, canonical names, private compiled caches. |
...
| Far-terrain LoD, advanced propagated/colored light, GI, volumetrics, OIT, reflection/refraction, advanced materials | Post-v1 unless release scope changes | Replaceable render snapshot/material boundary and ordinary fog at resident edge. |
| Combat rewind, support-loss grace, speculative collision, nested 40 Hz controller | Separate experiment after a demonstrated product failure | `ActionId`, revisions, bounded histories; all features remain disabled. |
...
- **SQLite, GNS, child-process hosting, mesher/backend, and pack details are candidates.** None is selected by this map; E2, E4, E5, E6, and E7 must record outcomes.
- **The 20 Hz world clock is the v1 baseline, not a passed feel test.** A nested 40 Hz controller remains locked until E3 records a blind 20 Hz failure.
...
G5 is the first point at which the packet permits the word “playable.” Passing one branch prototype supplies evidence only.
